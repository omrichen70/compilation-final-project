...
L_code_ptr_bin_apply:
        lea rsi, [rsp + (8 * 4)] ;rsi <-- address of 2nd param
        mov rax, qword [rsi] ;rax <-- holds the list we got, 2nd param
        mov rcx, 0 ;rcx <-- list length counter

        .L_find_list_len:
        cmp rax, sob_nil ;if rax is null, done with the list
        je .L_finish_list_len
        assert_pair(rax)
        mov rax, SOB_PAIR_CDR(rax) ;rax <-- cdr(rax)
        inc rcx
        jmp .L_find_list_len

        .L_finish_list_len:
        lea rbx, [(8 * rcx) + (8 * 2)] ;rbx points to where we want to place the params
        sub rsi, rbx 
        mov rbx, rsp
        mov rsp, rsi
        mov r8, qword [rbx + (8 * 3)] ;r8 <-- closure
        assert_closure(r8)
        mov r9, qword [rbx + (8 * 4)] ;r9 <-- list
        ;copy return address
        mov rax, qword [rbx]
        mov qword [rsi], rax
        add rsi, 8 * 1
        ;copy env of function
        mov rax, SOB_CLOSURE_ENV(r8)
        mov qword [rsi], rax
        add rsi, 8 * 1
        ;place list len = new args count
        mov qword [rsi], rcx
        add rsi, 8 * 1

        .L_copy_list_to_stack:
        cmp rcx, 0
        je .L_finish_copy_list_to_stack
        mov rax, SOB_PAIR_CAR(r9)
        mov qword [rsi], rax
        mov r9, SOB_PAIR_CDR(r9)
        add rsi, 8 * 1
        dec rcx
        jmp .L_copy_list_to_stack

        .L_finish_copy_list_to_stack:
        ...
        ; .L_finish_list_len:
        ; inc r8
        ; mov rcx, r8 ;now rcx holds the list of the length
        ; add rcx, 3
        ; shl rcx, 3 ;now rcx is (list_len + 3) * 8
        ; sub rsp, rcx ;enlarge the stack by (list_len + 3) * 8
        ; mov rcx, r8 ;rdx will be the index

        ; .L_copy_list_to_stack:
        ; cmp rcx, 0
        ; je .L_finish_copy_list_to_stack
        ; mov rdx, rcx
        ; shl rdx, 3 ;gets rdx to the right position to place param from list
        ; mov r11, r10
        ; sub r11, rdx ;now r11 points to the place where we want to place the param
        ; mov r12, SOB_PAIR_CAR(rsi)
        ; mov qword [r11], r12
        ...
        ; .L_finish_copy_list_to_stack:
        ; ;insert length of list
        ; sub r11, 8 * 2
        ; mov qword [r11], r8
        ; ;insert environment
        ; sub r11, 8 * 1
        ; mov r12, SOB_CLOSURE_ENV(rdi)
        ; mov qword [r11], r12
        ...
        ; mov r9, r8
        ; add r9, 3
        ; mov r11, r10 ;r11 = rbp to look under
        ; mov r12, r10 ;r12 = rbp to look above
        ; mov r13, 4
        ; shl r13, 3
        ; add r12, r13 ;now r12 points to the place where we want to put the param
        ; mov rdx, 8

        ; .L_shift_new_frame:
        ; cmp r9, 0
        ; je .L_bin_apply_finish
        ; sub r11, 8 ;holds the element we need to move
        ; mov r13, qword [r11]
        ; mov qword [r12], r13
        ; sub r12, 8
        ; dec r9
        ; jmp .L_shift_new_frame
